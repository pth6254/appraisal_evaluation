...
| 기능 | 설명 |
|------|------|
| **매수 검토 케이스** | 매수 목표·예산·관심 지역·후보를 묶고 시세·자금·권리 분석 결과와 가격·자금·권리·현장·계약 체크리스트를 후보별로 관리 |
...
├── tests/                          pytest 테스트 (28개 파일, 733개 — 접근제어·시장탐색·케이스 소유권 등 포함)
...
전체 733개 테스트 중 DB에 접근하지 않는 나머지는 Postgres·Redis 없이도 동작한다 —
